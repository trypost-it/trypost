...
- In Dusk tests, ALWAYS use named routes via `route()` helper. NEVER hardcode URLs like `'https://app.sendkit.test/login'`.
    - Example: `$browser->visit(route('login'))` instead of `$browser->visit('https://app.sendkit.test/login')`.
