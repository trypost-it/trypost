...
class ConnectionVerifier
{
    ...
    private function verifyLinkedIn(SocialAccount $account): bool
    {
        $response = Http::withToken($account->access_token)
            ...
            ->get(config('postpro.platforms.linkedin.api').'/rest/userinfo');
    ...
    private function verifyLinkedInPage(SocialAccount $account): bool
    {
        $response = Http::withToken($account->access_token)
            ...
            ->get(config('postpro.platforms.linkedin-page.api').'/rest/organizationAcls', [
                'q' => 'roleAssignee',
            ]);
    ...
    {
        $response = Http::withToken($account->access_token)
            ->get(config('postpro.platforms.x.api').'/users/me');
    ...
    {
        $response = Http::get(config('postpro.platforms.facebook.graph_api').'/me', [
            'fields' => 'id,name',
            'access_token' => $account->access_token,
        ]);
    ...
    {
        $response = Http::get(config('postpro.platforms.threads.graph_api').'/me', [
            'fields' => 'id,username',
            'access_token' => $account->access_token,
        ]);
    ...
    private function verifyTikTok(SocialAccount $account): bool
    {
        $response = Http::withToken($account->access_token)
            ...
            ->get(config('postpro.platforms.tiktok.api').'/user/info/', [
    ...
    {
        $response = Http::withToken($account->access_token)
            ->get(config('postpro.platforms.pinterest.api').'/user_account');
