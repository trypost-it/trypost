...
const isBlocked = (plan: Plan): boolean =>
    !isCurrent(plan) && downgradeBlocker(plan) !== null;
...
const ctaLabel = (plan: Plan): string => {
    ...
    if (hasActiveSubscription.value)
        return trans('billing.upgrade_dialog.switch');
...
const handleSelect = (plan: Plan) => {
    ...
    const url = hasActiveSubscription.value
        ? swap.url(plan.id)
        : checkout.url(plan.id);
        ...
        <DialogContent class="w-[95vw] gap-0 p-0 sm:max-w-6xl">
            <TooltipProvider :delay-duration="150">
                <div class="px-6 pt-6 text-center">
                    ...
                    <DialogDescription
                        class="mx-auto mt-2 max-w-2xl text-base text-balance text-foreground/70"
                    >
                ...
                <div class="flex justify-center pt-6 pb-2">
                    <div class="relative">
                        ...
                        <span
                            :class="[
                                'pointer-events-none absolute top-1/2 left-full ml-3 inline-flex -translate-y-1/2 -rotate-3 items-center gap-1 rounded-md border-2 border-foreground bg-amber-200 px-2.5 py-1 text-[10px] font-black tracking-widest whitespace-nowrap text-foreground uppercase shadow-xs transition-all',
                                isYearly
                                    ? 'opacity-100'
                ...
                <!-- Plan cards -->
                <div
                    class="grid grid-cols-1 gap-5 px-6 pt-8 pb-6 md:grid-cols-2 xl:grid-cols-4"
                >
                    <div
                        v-for="plan in plans"
                        :key="plan.id"
                    ...
                    >
                        ...
                        <span
                            ...
                            class="absolute -top-3.5 left-1/2 inline-flex -translate-x-1/2 items-center rounded-md border-2 border-foreground bg-card px-3 py-1 text-[11px] font-black tracking-widest whitespace-nowrap text-foreground uppercase shadow-2xs"
                        >
                        ...
                        <div class="mb-2">
                            <div class="flex items-baseline gap-1">
                                <span
                                    class="text-4xl font-black text-foreground tabular-nums"
                                >
