        ...
        if (isVideo) {
            if (rules.maxVideoBytes && size > rules.maxVideoBytes) {
                return {
                    key: 'video_too_large',
                    params: {
                        max: formatBytes(rules.maxVideoBytes),
            ...
            if (
                ...
                return {
                    key: 'video_too_long',
                    params: {
                        max: formatDuration(rules.maxVideoDurationSec),
            ...
            return {
                key: 'image_too_large',
                params: {
                    max: formatBytes(rules.maxImageBytes),
                    current: formatBytes(size),
                },
            };
        }
...
export const getMediaItemIssue = (
    ...
    if (isVideo && rules.maxVideoBytes && size > rules.maxVideoBytes)
        return 'video_too_large';
    if (!isVideo && rules.maxImageBytes && size > rules.maxImageBytes)
