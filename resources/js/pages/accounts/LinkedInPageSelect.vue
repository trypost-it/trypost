...
<template>
    <PopupLayout :title="$t('accounts.linkedin.title')">
        ...
        >
            ...
            <input
                type="hidden"
                name="organization_vanity_name"
                :value="selectedOrg?.vanity_name ?? ''"
            />
            <input
                type="hidden"
                name="organization_logo"
                :value="selectedOrg?.logo ?? ''"
            />
        </form>
        ...
        <div class="flex flex-col gap-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold tracking-tight">
                        {{ $t('accounts.linkedin.title') }}
                    </h1>
                    <p class="text-sm text-muted-foreground">
                        {{ $t('accounts.linkedin.description') }}
                    </p>
                </div>
            </div>
            ...
            <div
                v-if="organizations.length === 0 && !error"
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p v-else class="text-sm text-muted-foreground">
                                {{ $t('accounts.linkedin.page_label') }}
                            </p>
                        </div>
                        <div
